...
class Polar {
    ...
    fun init(context: Context, listener: DeviceStateListener) {
        ...
        Logger.info(TAG, "version: " + PolarBleApiDefaultImpl.versionInfo())
        ...
        api.setApiCallback(object : PolarBleApiCallback() {
            ...
            override fun hrNotificationReceived(identifier: String, data: PolarHrData) {
                ...
                listener.hrDataReceived(HRData(data.hr, data.rrsMs, data.rrs))
            }
        ...
        if (!this::broadcastDisposable.isInitialized || broadcastDisposable.isDisposed) {
            broadcastDisposable = api.startListenForPolarHrBroadcasts(null)
                .subscribe(
                    ...
                    { error: Throwable ->
                        Logger.error(TAG, "Broadcast listener failed. Reason $error")
                    },
    ...
    fun connect(deviceId: String) {
        if (deviceConnected) {
            ...
            try {
                ...
                Logger.error(TAG, "Failed to connect. Reason $polarInvalidArgument ")
            }
        }
    }
    ...
    fun disconnect() {
        if (!deviceConnected) {
            ...
            try {
                ...
                Logger.error(TAG, "Failed to disconnect. Reason $polarInvalidArgument ")
            }
        }
    }
    ...
    fun autoConnect() {
        ...
        autoConnectDisposable = api.autoConnectToDevice(-50, "180D", null)
            .subscribe(
                ...
                { throwable: Throwable -> Logger.error(TAG, "" + throwable.toString()) }
            )
    }
        ...
        if (isDisposed) {
            scanDisposable = api.searchForDevice()
                ...
                .subscribe(
                    ...
                    { error: Throwable ->
                        Logger.error(TAG, "Device scan failed. Reason $error")
                    },
                    { Logger.debug(TAG, "complete") }
                )
        } else {
            ...
            ecgDisposable = api.startEcgStreaming(_deviceId, PolarSensorSetting(settings))
                .subscribe(
                    { polarEcgData: PolarEcgData ->
                        ...
                        listener.ecgDataReceived(ECGData(polarEcgData.timeStamp,
                            polarEcgData.samples))
                    },
                    { error: Throwable ->
                        Logger.error(TAG, "ECG stream failed. Reason $error")
                    },
            ...
            accDisposable = api.startAccStreaming(_deviceId, PolarSensorSetting(settings))
                ...
                .subscribe(
                    { polarAccelerometerData: PolarAccelerometerData ->
                        ...
                        listener.accDataReceived(ACCData(polarAccelerometerData.timeStamp,
                            polarAccelerometerData.samples.map{data -> Point(data.x, data.y, data.z) }))
                    },
                    { error: Throwable ->
                        Logger.error(TAG, "ACC stream failed. Reason $error")
                    },
                    {
